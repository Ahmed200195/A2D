...
SELECT * FROM EMP WHERE (SUBSTR(ENAME, 1, 1) = SUBSTR(UPPER(ENAME), LENGTH(ENAME), 1) 
...
SELECT EMPNO FROM EMP WHERE EMPNO NOT IN (SELECT EMPNO FROM VAC WHERE TO_CHAR(SDATE, 'YYYY') = 2020 AND TO_CHAR(SDATE, 'MM') > 6);
SELECT ENAME, EMPNO FROM EMP WHERE EMPNO IN 
(SELECT EMPNO FROM VAC WHERE TO_CHAR(SDATE, 'YYYY') = TO_CHAR(SYSDATE, 'YYYY') 
GROUP BY EMPNO HAVING SUM(EDATE - SDATE) > 20);
SELECT E.ENAME, E.SAL, M.ENAME, M.JOB FROM EMP E, EMP M
WHERE M.EMPNO = E.MGR AND (E.SAL > 10000 OR E.JOB = 'ANALYST');
SELECT COUNT(*), SUM(SAL), GRADE FROM EMP, SALGRADE WHERE SAL(+) BETWEEN LOSAL AND HISAL
GROUP BY GRADE;
SELECT C.DNAME, S.DNAME FROM DEPT C, DEPT S WHERE C.DEPTNO IN (SELECT DEPTNO FROM EMP GROUP BY DEPTNO 
HAVING COUNT(*) = (SELECT MAX(COUNT(*)) FROM EMP GROUP BY DEPTNO)) AND
S.DEPTNO IN (SELECT DEPTNO FROM EMP GROUP BY DEPTNO 
HAVING SUM(SAL) = (SELECT MAX(SUM(SAL)) FROM EMP GROUP BY DEPTNO));
DELETE FROM VAC WHERE TO_DATE(SDATE, 'DD/MM/YYYY') BETWEEN TO_DATE('12/3/2020', 'DD/MM/YYYY') AND TO_DATE('15/5/2020', 'DD/MM/YYYY');
UPDATE EMP E SET SAL = SAL * 1.15 WHERE SAL < (SELECT AVG(SAL) FROM EMP WHERE DEPTNO = E.DEPTNO);
UPDATE EMP SET SAL = DECODE(DEPTNO, 10, SAL * 1.1, 20, SAL * 1.15, SAL * 1.05);
INSERT INTO BONUS SELECT ENAME, SAL, JOB, COMM FROM EMP WHERE JOB IS NULL OR DEPTNO IS NULL;