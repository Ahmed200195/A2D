...
CREATE VIEW V1 AS
SELECT EMPNO, SDATE FROM VAC GROUP BY EMPNO, SDATE
HAVING SUM(EDATE - SDATE) IN (SELECT MAX(SUM(EDATE - SDATE)) FROM VAC
WHERE EMPNO = 190 GROUP BY TO_CHAR(SDATE, 'DD/MM/YYYY'));
CREATE VIEW V2 AS
SELECT ENAME, EMP.EMPNO, SAL, SUM(EDATE - VAC.SDATE), TO_CHAR(V1.SDATE, 'YYYY') 
FROM EMP, V1 WHERE EMP.EMPNO = V1.EMPNO
GROUP BY ENAME, EMP.EMPNO, SAL, TO_CHAR(V1.SDATE, 'YYYY');
