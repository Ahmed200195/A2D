...
DESC EMP
GRANT REFERENCES ON EMP TO HME
GRANT INSERT(ENAME) ON EMP TO HME
REVOKE INSERT ON EMP FROM HME CASCADE CONSTRAINTS
SELECT * FROM USER_TAB_PRIVS_MADE
SELECT * FROM USER_COL_PRIVS_MADE
SELECT * FROM USER_SOURC
SELECT * FROM USER_TABLES;
SELECT * FROM USER_USERS;
(انشئ اجرائية يمرر لها رقم موظف وتعيد اسم المدير ومعاش المدير وعالج عن طريق اكسبشن
 كون رقم الموظف غير موجود أو ليس له مدير)
CREATE PROCEDURE GETMGR(ENUM NUMBER, MNAME OUT VARCHAR2, MSAL OUT NUMBER) IS
X NUMBER(1);
NUM NUMBER;
BEGIN
X := 0;
SELECT EMPNO INTO NUM FROM EMP WHERE EMPNO = ENUM;
X := 1;
SELECT M.ENAME, M.SAL INTO MNAME, MSAL FROM EMP E, EMP M WHERE M.EMPNO = E.MGR AND
E.EMPNO = ENUM;
EXCEPTION
WHEN NO_DATA_FOUND THEN
IF X = 0 THEN
...
ELSIF X = 1 THEN
DBMS_OUTPUT.PUT_LINE('THIS EMPNO NO MGR');
END IF;
END;
SET SERVEROUTPUT ON
DECLARE
...
END;
تابع يمرر له رقم قسم ما ويرد اسم القسم وعدد الموظفين ومجموع المعاشات،
 عالج عن طريق اكسبشن كون رقم القسم غير موجود في جدول 
 emp او كون رقم القسم غير موجود في جدول dept
CREATE OR REPLACE FUNCTION FUN2(D NUMBER) RETURN VARCHAR2 IS
EX NUMBER(1) := 0;
DNO NUMBER;
X VARCHAR2(100);
BEGIN
SELECT DEPTNO INTO DNO FROM DEPT WHERE DEPTNO = D;
EX := 1;
SELECT DNAME || ' ' || SUM(SAL) || ' ' || COUNT(*) INTO X FROM EMP, DEPT WHERE EMP.DEPTNO = DEPT.DEPTNO AND
EMP.DEPTNO = D GROUP BY DNAME;
RETURN X;
EXCEPTION
WHEN NO_DATA_FOUND THEN
IF EX = 0 THEN
RETURN 'NOT FOUND DEPTNO FROM DEPT';
ELSIF EX = 1 THEN
RETURN 'NOT FOUND DEPTNO FROM EMP';
END IF;
END;
SELECT FUN2(200) FROM DUAL
CREATE ROLE R1
CREATE ROLE R2
GRANT R2 TO R1
SELECT SUM(SAL)||COUNT(*) FROM EMP WHERE DEPTNO = 80;

--SHOW USERS INCLUDE PDBXE1
SELECT * FROM ALL_USERS
--SHOW USER PROPERTIES MY OWN
SELECT * FROM USER_USERS
--SHOW DICTIONARY
SELECT * FROM DICT WHERE TABLE_NAME LIKE '%PRIV%'
--SHOW PRIVILEGES SYSTEM
SELECT * FROM USER_SYS_PRIVS
--SHOW PRIVILEGES SYSTEM INCLUDE ROLE
SELECT * FROM ROLE_SYS_PRIVS
--SHOW ROLES
SELECT * FROM USER_ROLE_PRIVS
--SHOW ROLES INCLUD ROLE OTHER
SELECT * FROM ROLE_ROLE_PRIVS
--SHOW PRIVILEGES ENABLED INCLUD CURRENT SESSION 
SELECT * FROM SESSION_PRIVS
...
END LOOP;
END;
CREATE FUNCTION FUN3 RETURN VARCHAR2 IS
BEGIN
SELECT DNAME FROM EMP E, DEPT D WHERE E.DEPTNO = D.DEPTNO
GROUP BY DNAME HAVING COUNT(*) = (SELECT MAX(COUNT(*)) FROM EMP GROUP BY DEPTNO);
IF SQL%ROWCOUNT > 1 THEN
SELECT DNAME FROM EMP E, DEPT D WHERE E.DEPTNO = D.DEPTNO
GROUP BY DNAME HAVING COUNT(*) = (SELECT MIN(COUNT(*)) FROM EMP GROUP BY DEPTNO)
END IF;
END;
SELECT COUNT(*) FROM EMP WHERE DEPTNO = 80
CREATE PROCEDURE PRO3(ENUM NUMBER, SDAY OUT NUMBER) IS 
CURSOR C1 IS SELECT SDATE, EDATE FROM VAC WHERE EMPNO = ENUM;
BEGIN
FOR I IN C1
LOOP
DBMS_OUTPUT.PUT_LINE(I.SDATE || ' ' || I.EDATE);
END LOOP;
SELECT SUM(EDATE - SDATE) INTO SDAY FROM EMP WHERE EMPNO = ENUM;
EXCEPTION
WHEN NO_DATA_FOUND THEN
DBMS_OUTPUT.PUT_LINE('NO VAC');
END;
CREATE FUNCTION FUN3(NO NUMBER) RETURN VARCHAR2 IS
X VARCHAR2(50);
BEGIN
SELECT DNAME INTO X FROM DEPT WHERE DEPTNO = NO;
RETURN X;
EXCEPTION
WHEN NO_DATA_FOUND THEN
RETURN 'NO DEPT';
