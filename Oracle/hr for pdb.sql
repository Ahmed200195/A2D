...
INSERT INTO VAC VALUES(190, TO_DATE('10/01/2021', 'DD/MM/YYYY'), TO_DATE('30/01/2021', 'DD/MM/YYYY'))
BEGIN
INSERT INTO VAC VALUES(190, TO_DATE('07/08/2021', 'DD/MM/YYYY'), TO_DATE('18/09/2021', 'DD/MM/YYYY'));
INSERT INTO VAC VALUES(180, TO_DATE('21/03/2021', 'DD/MM/YYYY'), TO_DATE('18/10/2021', 'DD/MM/YYYY'));
INSERT INTO VAC VALUES(170, TO_DATE('11/06/2021', 'DD/MM/YYYY'), TO_DATE('07/08/2021', 'DD/MM/YYYY'));
INSERT INTO VAC VALUES(160, TO_DATE('18/09/2021', 'DD/MM/YYYY'), TO_DATE('09/12/2021', 'DD/MM/YYYY'));
INSERT INTO VAC VALUES(150, TO_DATE('09/12/2021', 'DD/MM/YYYY'), TO_DATE('30/01/2022', 'DD/MM/YYYY'));
END;
SELECT * FROM VAC
DROP TABLE VVAC
...
PROCEDURE INS_COPY_EMP(PK NUMBER, NAME VARCHAR2);
...
END;
DESC COPY_DEPT
CREATE OR REPLACE TRIGGER TR BEFORE INSERT ON COPY_DEPT FOR EACH ROW WHEN (NEW.DEPTNO IS NULL)
DECLARE
CNT NUMBER;
CURSOR C1 IS SELECT DEPTNO FROM COPY_DEPT;
BEGIN
...
FOR I IN 1..9999 LOOP 
        ...
        END IF; 
        END LOOP;
        IF CNT IS NOT NULL THEN :NEW.DEPTNO := CNT; EXIT; END IF;
END LOOP;
END;
DELETE FROM COPY_DEPT WHERE DNAME = 'CVBN'
DELETE FROM COPY_DEPT WHERE DEPTNO = 10
INSERT INTO COPY_DEPT VALUES(NULL, 'CVBN', 400, 2342);
SHOW ERROR
SELECT * FROM SESSION_PRIVS
SELECT * FROM COPY_DEPT ORDER BY 1
SELECT * FROM COPY_EMP
CREATE VIEW VCOPY AS SELECT DEPTNO, MGR FROM COPY_DEPT
SELECT * FROM VCOPY
DELETE FROM COPY_DEPT WHERE DEPTNO = 3;
SET SERVEROUTPUT ON
DECLARE
X NUMBER := 0;
BEGIN
...
END;
SELECT * FROM EMP WHERE EMPNO IN (SELECT MGR FROM EMP WHERE EMPNO = 160)
SELECT * FROM EMP WHERE EMPNO = 160
CREATE OR REPLACE FUNCTION FUN1 RETURN VARCHAR2 IS
DN VARCHAR2(100);
DNO NUMBER;
BEGIN
SELECT MIN(DEPTNO) INTO DNO FROM DEPT WHERE DEPTNO IN (SELECT DEPTNO FROM EMP GROUP BY DEPTNO
HAVING COUNT(*) = (SELECT MAX(COUNT(*)) FROM EMP GROUP BY DEPTNO));
...
TEXT VARCHAR2(1000);
BEGIN
MYFILE := UTL_FILE.FOPEN('DIR', 'Sample.txt', 'R');
UTL_FILE.GET_LINE(MYFILE, TEXT);
DBMS_OUTPUT.PUT_LINE(TEXT);
UTL_FILE.FCLOSE(MYFILE);
END;
CREATE OR REPLACE PACKAGE EMP_PAK IS
TYPE REC IS RECORD(EN EMP.ENAME%TYPE, JO EMP.JOB%TYPE, HR EMP.HIRE_DATE%TYPE);
TYPE TAB IS TABLE OF REC INDEX BY BINARY_INTEGER;
PROCEDURE READ_EMP;
PROCEDURE WRITE_EMP;
END;
CREATE OR REPLACE PACKAGE BODY EMP_PAK IS
        MYTAB TAB;
        CURSOR C1 IS SELECT ENAME, EMPNO, JOB, HIRE_DATE FROM EMP;
        PROCEDURE READ_EMP IS
        BEGIN
        FOR I IN C1 LOOP
        MYTAB(I.EMPNO).EN := I.ENAME;
        MYTAB(I.EMPNO).JO := I.JOB;
        MYTAB(I.EMPNO).HR := I.HIRE_DATE;
        END LOOP;
        END;
        PROCEDURE WRITE_EMP IS
        X NUMBER := MYTAB.FIRST;
        BEGIN
        FOR I IN 1..MYTAB.COUNT LOOP
        DBMS_OUTPUT.PUT_LINE(MYTAB(X).EN || ' ' || MYTAB(X).JO || ' ' || MYTAB(X).HR);
        X := MYTAB.NEXT(X);
        END LOOP;
        END;
END;
...
EMP_PAK.WRITE_EMP;
END;
CREATE OR REPLACE PACKAGE PAK2 IS
PROCEDURE PRO1(NUM NUMBER, REC_NUM OUT EMP%ROWTYPE);
D DATE;
PROCEDURE PRINT_DATE;
END;
CREATE OR REPLACE PACKAGE BODY PAK2 IS
CURSOR C1 IS SELECT ENAME FROM EMP WHERE DEPTNO = 10;
D := SYSDATE;
PROCEDURE PRO1(NUM NUMBER, REC_NUM OUT EMP%ROWTYPE) IS
BEGIN
SELECT * INTO REC_NUM FROM EMP WHERE EMPNO = NUM;
END;
...
END LOOP;
END;
CREATE OR REPLACE PACKAGE PAK3 IS
PROCEDURE PRO1(DNUM NUMBER);
TYPE REC IS RECORD(DN DEPT.DNAME%TYPE, CNT NUMBER, SUMSAL NUMBER);
FUNCTION FUN1(DNUM NUMBER) RETURN REC;
END;
CREATE OR REPLACE PACKAGE BODY PAK3 IS
PROCEDURE PRO1(DNUM NUMBER) IS
...
SELECT MAX(DEPTNO) FROM COPY_DEPT

CREATE PROCEDURE SAVE_EMP IS
TYPE REC IS RECORD(E EMP%ROWTYPE, CNT NUMBER);
...
FOR I IN C1 LOOP
TAB(I.EMPNO).E := I;
SELECT SUM(EDATE - SDATE) INTO TAB(I.EMPNO).CNT FROM VAC
WHERE EMPNO = I.EMPNO;
END LOOP;
END;
DROP PROCEDURE SAVE_EMP

CREATE TRIGGER TR BEFORE INSERT ON EMP FOR EACH ROW WHEN (NEW.EMPNO IS NULL)
DECLARE
CNT NUMBER;
MX NUMBER;
MN NUMBER;
BEGIN
SELECT MAX(EMPNO) INTO MX FROM EMP;
SELECT MIN(EMPNO) INTO MN FROM EMP;
FOR I IN MN..MX LOOP 
    CNT := I;
    SELECT EMPNO INTO CNT FROM EMP WHERE EMPNO = CNT;
END LOOP;
EXCEPTION
WHEN NO_DATA_FOUND THEN
:NEW.EMPNO := CNT;
...
SET SERVEROUTPUT ON
SET AUTOCOMMIT OFF;
INSERT INTO COPY_EMP VALUES(7)
SELECT * FROM COPY_EMP WHERE EMPNO = 7
ROLLBACK
BEGIN
DBMS_OUTPUT.PUT_LINE('ASDF');
END;
SELECT * FROM EMP
CREATE OR REPLACE FUNCTION FUN1(EN NUMBER) RETURN VARCHAR2 IS
TEM VARCHAR2(100);
BEGIN
SELECT ENAME INTO TEM FROM EMP WHERE EMPNO = EN;
RETURN TEM;
END;
CREATE OR REPLACE FUNCTION FUN2(EN VARCHAR2) RETURN NUMBER IS
TEM NUMBER;
BEGIN
SELECT EMPNO INTO TEM FROM EMP WHERE ENAME = EN;
RETURN TEM;
END;
SELECT FUN2(FUN1(102)) FROM DUAL
SELECT FUN1(102) FROM DUAL
SELECT DISTINCT ENAME FROM EMP 

--------------الدفعة الاولى--------------
--------------------------------------------------------------------------------------------
CREATE PACKAGE PACK IS
TYPE REC1 IS RECORD(DN DEPT.DNAME%TYPE, CNT NUMBER);
TYPE T IS TABLE OF REC INDEX BY BINARY_INTEGER;
PROCEDURE SAVE_DEPT;
TYPE REC2 IS RECORD(EN EMP.ENAME%TYPE, JB EMP.JOB%TYPE, SL EMP.SAL%TYPE);
FUNCTION GET_EMP(ENO NUMBER) RETURN REC2;
END PACK;
CREATE OR REPLACE PACKAGE BODY PACK IS
PROCEDURE SAVE_DEPT IS
CURSOR C1 IS SELECT DNAME, DEPT.DEPTNO, COUNT(*) EMPCNT FROM EMP, DEPT WHERE DEPT.DEPTNO = EMP.DEPTNO
GROUP BY DNAME, DEPT.DEPTNO;
MYT T;
BEGIN
FOR I IN C1 LOOP
MYT(I.DEPTNO).DN := I.DNAME;
MYT(I.DEPTNO).CNT := I.EMPCNT;
END LOOP;
END SAVE_DEPT;
FUNCTION GET_EMP(ENO NUMBER) RETURN REC2 IS
MYR REC2;
BEGIN
SELECT ENAME, JOB, SAL INTO MYR.EN, MYR.JB, MYR.SL FROM EMP;
RETURN MYR;
END GET_EMP;
...
MYREC PACK.REC2;
BEGIN
MYREC := PACK.GET_EMP(7800);
END;
--------------------------------------------------------------------------------------------
CREATE TRIGGER TR BEFORE INSERT ON VAC FOR EACH ROW
DECLARE
X NUMBER;
BEGIN
IF :NEW.EMPNO IS NULL THEN
RAISE_APPLICATION_ERROR(-20000, 'CAN NOT INSERT..EMPNO IS NULL');
ELSE
SELECT EMPNO INTO X FROM EMP WHERE EMPNO = :NEW.EMPNO;
END IF;
IF :NEW.SDATE IS NULL THEN
:NEW.SDATE := SYSDATE;
END IF;
IF :NEW.EDATE IS NULL THEN
:NEW.EDATE := :NEW.SDATE + 1;
END IF;
EXCEPTION WHEN NO_DATA_FOUND THEN
RAISE_APPLICATION_ERROR(-20001, 'CAN NOT INSERT..EMPNO NOT FOUNT');
END TR;
--------------------------------------------------------------------------------------------
CREATE PACKAGE PACK IS
TYPE REC IS RECORD(EN EMP.ENAME%TYPE, VCNT NUMBER);
TYPE T IS TABLE OF REC INDEX BY BINARY_INTEGER;
PROCEDURE GET_EVAC(MYT OUT T);
PROCEDURE SAVE_EMP(DNO NUMBER);
END PACK;
CREATE OR REPLACE PACKAGE BODY PACK IS
PROCEDURE GET_EVAC(MYT OUT T) IS
CURSOR C1 IS SELECT ENAME, EMP.EMPNO, SUM(EDATE - SDATE) SUMVAC FROM EMP, VAC WHERE EMP.EMPNO = VAC.EMPNO
AND TO_CHAR(SDATE, 'YYYY') = 2020 GROUP BY ENAME, EMP.EMPNO;
BEGIN
FOR I IN C1 LOOP
MYT(I.EMPNO).EN := I.ENAME;
MYT(I.EMPNO).VCNT := I.SUMVAC;
END LOOP;
END GET_EVAC;
PROCEDURE SAVE_EMP(DNO NUMBER) IS
CURSOR C1 IS SELECT ENAME, SAL FROM EMP WHERE DEPTNO = DNO;
FIL UTL_FILE.FILE_TYPE;
BEGIN
FIL := UTL_FILE.FOPEN('DIR', 'T.TXT', 'W');
FOR I IN C1 LOOP
UTL_FILE.PUT_LINE(FIL, I.ENAME || I.SAL);
END LOOP;
UTL_FILE.FCLOSE(FIL);
END SAVE_EMP;
END PACK;
...
END;
--------------------------------------------------------------------------------------------
CREATE TRIGGER TR BEFORE CREATE OR DROP OR ALTER ON SCHEMA
BEGIN
IF ORA_SYSEVENT = 'CREATE' AND ORA_DICT_OBJ_TYPE = 'TABLE' 
AND TO_CHAR(SYSDATE, 'HH24') BETWEEN 8 AND 14 THEN
RAISE_APPLICATION_ERROR(-20000, 'NO CREATE TABLE EMP');
ELSIF ORA_SYSEVENT = 'ALTER' AND ORA_DICT_OBJ_TYPE = 'TABLE' AND ORA_DICT_OBJ_NAME = 'EMP'
AND ORA_IS_DROP_COLUMN = 'EMPNO' THEN 
RAISE_APPLICATION_ERROR(-20000, 'NO ALTER TABLE EMP');
ELSIF ORA_SYSEVENT = 'DROP' AND ORA_DICT_OBJ_TYPE = 'TABLE' THEN
RAISE_APPLICATION_ERROR(-20000, 'NO DROP ANY TABLE');
ELSIF ORA_SYSEVENT IN ('CREATE', 'DROP') AND ORA_DICT_OBJ_TYPE = 'SEQUENCE' THEN
RAISE_APPLICATION_ERROR(-20000, 'NO CREATE OR DROP ANY SEQUENCE');
END TR;
--------------------------------------------------------------------------------------------
CREATE PACKAGE PACK IS
PROCEDURE PRINT;
TYPE REC IS RECORD(DNO DEPT.DEPTNO%TYPE, DN DEPT.DNAME%TYPE, SMSAL NUMBER);
TYPE T IS TABLE OF REC INDEX BY BINARY_INTEGER;
MYTAB T;
PROCEDURE SAVE_DEPT;
END PACK;
...
END PRINT;
PROCEDURE SAVE_DEPT IS
CURSOR C1 IS SELECT DEPT.DEPTNO, DNAME, SUM(SAL) SUMSAL FROM EMP, DEPT WHERE EMP.DEPTNO = DEPT.DEPTNO
GROUP BY DEPT.DEPTNO, DNAME;
BEGIN
FOR IN IN C1 LOOP
MYTAB(I.DEPTNO).DNO := I.DEPTNO;
MYTAB(I.DEPTNO).DN := I.DNAME;
MYTAB(I.DEPTNO).SMSAL := I.SUMSAL;
END LOOP;
END SAVE_DEPT;
BEGIN
D := SYSDATE; X := X + 1;
...
PACK.SAVE_DEPT;
X := PACK.MYTAB.FIRST;
FOR I IN 1..PACK.MYTAB.COUNT LOOP
DBMS_OUTPUT.PUT_LINE(PACK.MYTAB(X).DNO || PACK.MYTAB(X).DN || PACK.MYTAB(X).SMSAL);
X := PACK.MYTAB.NEXT(X);
END LOOP;
END;
--------------------------------------------------------------------------------------------
CREATE TRIGGER TR AFTER GRANT ON DATABASE
DECLARE
PN NUMBER; P ORA_NAME_LIST_T; FIL UTL_FILE.FILE_TYPE;
BEGIN
PN := ORA_PRIVILEGE_LIST(P);
FIL := UTL_FILE.FOPEN('DIR', 'T.TXT', 'W');
FOR I IN PN LOOP
IF P(I) IN ('INSERT', 'UPDATE', 'DELETE') AND ORA_DICT_OBJ_TYPE = 'TABLE'
AND ORA_DICT_OBJ_NAME = 'EMP' THEN
UTL_FILE.PUT_LINE(FIL, P(I) || ORA_DICT_OBJ_TYPE || ORA_DICT_OBJ_NAME || SYSDATE);
END IF;
END LOOP;
UTL_FILE.FCLOSE(FIL);
END TR;
...
CREATE PACKAGE PACK IS
PROCEDURE PRINT;
TYPE REC IS RECORD(SD VAC.SDATE%TYPE, ED VAC.EDATE%TYPE);
TYPE T IS TABLE OF REC INDEX BY BINARY_INTEGER;
MYTAB T;
FUNCTION GET_VAC(EN NUMBER) RETURN T;
END PACK;

CREATE OR REPLACE PACKAGE BODY PACK IS
TYPE REC2 IS RECORD(ST DATE, CN NUMBER);
MYREC2 REC2;
PROCEDURE PRINT IS
BEGIN
DBMS_OUTPUT.PUT_LINE(REC2.ST || MYREC2.CN);
END PRINT;
FUNCTION GET_VAC(EN NUMBER) RETURN T IS
CURSOR C1 IS SELECT EMPNO, SDATE, EDATE FROM VAC WHERE EMPNO = EN;
BEGIN
FOR I IN C1 LOOP
MYTAB(I.EMPNO).SD := I.SDATE;
MYTAB(I.EMPNO).ED := I.EDATE;
END LOOP;
RETURN MYTAB;
END GET_VAC;
...
X NUMBER;
BEGIN
PACK.MYTAB := PACK.GET_VAC(7800);
X := PACK.MYTAB.FIRST;
FOR I IN 1..PACK.MYTAB.COUNT LOOP
DBMS_OUTPUT.PUT_LINE(PACK.MYTAB(X).SD || PACK.MYTAB(X).ED);
X := PACK.MYTAB.NEXT(X);
END LOOP;
END;
--------------------------------------------------------------------------------------------
CREATE PACKAGE PACK IS
TYPE REC IS RECORD(EN EMP.ENAME%TYPE, DN DEPT.DNAME%TYPE, GD SALGRADE.GRADE%TYPE);
PROCEDURE GET_REC(E NUMBER, MYREC OUT REC);
TYPE REC2 IS RECORD(EN EMP.ENAME%TYPE, CNTVAC NUMBER);
TYPE T IS TABLE OF REC2 INDEX BY BINARY_INTEGER;
MYTAB T;
PROCEDURE SAVE_VAC;
PROCEDURE PRINT;
FUNCTION GET_CNTEMP(DNO NUMBER) RETURN NUMBER;
END PACK;

CREATE OR REPLACE PACKAGE BODY PACK IS

PROCEDURE GET_REC(E NUMBER, MYREC OUT REC) IS
BEGIN
SELECT ENAME, DNAME, GRADE INTO MYREC.EN, MYREC.DN, MYREC.GD FROM EMP E, DEPT D, SALGRADE
WHERE D.DEPTNO = E.DEPTNO AND SAL BETWEEN LOSAL AND HISAL AND EMPNO = E;
EXCEPTION
WHEN NO_DATA_FOUND THEN
DBMS_OUTPUT.PUT_LINE('NO DATA');
END GET_REC;

PROCEDURE SAVE_VAC IS
CURSOR C1 IS SELECT ENAME, DEPTNO, SUM(EDATE - SDATE) SUMVAC FROM EMP E, VAC V
WHERE E.EMPNO = V.EMPNO AND TO_CHAR(SDATE, 'YYYY') = 2022 GROUP BY ENAME, DEPTNO;
BEGIN
FOR I IN C1 LOOP
MYTAB(I.DEPTNO).EN := I.ENAME;
MYTAB(I.DEPTNO).CNTVAC := I.SUMVAC;
END LOOP;
END SAVE_VAC;
...
BEGIN
FOR I IN 1..MYTAB.COUNT LOOP
DBMS_OUTPUT.PUT_LINE(MYTAB(X).EN || ' : ' || MYTAB(X).CNTVAC);
X := MYTAB.NEXT(X);
END LOOP;
END PRINT;

FUNCTION GET_CNTEMP(DNO NUMBER) RETURN NUMBER IS
X NUMBER := 0; DCNT NUMBER;
BEGIN
SELECT DEPTNO INTO DCNT FROM DEPT;
X := X + 1;
SELECT COUNT(*) INTO DCNT FROM EMP, DEPT WHERE EMP.DEPTNO = 
DEPT.DEPTNO AND EMP.DEPTNO = DNO GROUP BY EMP.DEPTNO;
RETURN DCNT;
EXCEPTION
WHEN NO_DATA_FOUND THEN
IF X = 0 THEN
...
MYREC PACK.REC;
BEGIN
PACK.GET_REC(7800, MYREC);
END;
--------------------------------------------------------------------------------------------
CREATE TRIGGER TR BEFORE INSERT ON EMP FOR EACH ROW
BEGIN
IF :NEW.DEPTNO IS NULL THEN
:NEW.DEPTNO := 40;
END IF;
SELECT DEPTNO INTO :NEW.DEPTNO FROM DEPT WHERE DEPTNO = :NEW.DEPTNO;
SELECT MAX(EMPNO) INTO :NEW.EMPNO FROM EMP;
:NEW.EMPNO := NVL(:NEW.EMPNO, 0) + 1;
WHEN NO_DATA_FOUND THEN
RAISE_APPLICATION_ERROR(-20000, 'NOT FOUND IN DEPT');
END TR;
--------------------------------------------------------------------------------------------
/*
CREATE OR REPLACE TRIGGER TR1 BEFORE INSERT ON COPY_EMP FOR EACH ROW
BEGIN
SELECT DEPTNO INTO :NEW.DEPTNO FROM DEPT WHERE DEPTNO = :NEW.DEPTNO;
SELECT MAX(EMPNO) INTO :NEW.EMPNO FROM COPY_EMP;
:NEW.EMPNO := NVL(:NEW.EMPNO, 0) + 1;
EXCEPTION
WHEN NO_DATA_FOUND THEN
RAISE_APPLICATION_ERROR(-20000, 'NOT FOUND');
END;
INSERT INTO COPY_EMP(DEPTNO) VALUES(190);
SELECT * FROM COPY_EMP
*/
SELECT SUM(SAL) FROM EMP WHERE DEPTNO = 1000 GROUP BY DEPTNO
--------------------------------------------------------------------------------------------
CREATE PROCEDURE PRINT_VAC(EN NUMBER, SUMVAC OUT NUMBER) IS
CURSOR C1 IS SELECT SDATE, EDATE FROM VAC WHERE EMPNO = EN;
BEGIN
FOR I IN C1 LOOP
DBMS_OUTPUT.PUT_LINE(I.SDATE || ' ' || I.EDATE);
END LOOP;
SELECT SUM(EDATE - SDATE) INTO SUMVAC FROM VAC WHERE EMPNO = EN GROUP BY EMPNO;
EXCEPTION
WHEN NO_DATA_FOUND THEN
DBMS_OUTPUT.PUT_LINE('NO VAC');
END;
--------------------------------------------------------------------------------------------
CREATE FUNCTION GET_DNAME(DNO NUMBER) RETURN VARCHAR2 IS
DN VARCHAR2(50);
BEGIN
SELECT DNAME INTO DN FROM DEPT WHERE DEPTNO = DNO;
RETURN DN;
...
END;
--------------------------------------------------------------------------------------------
CREATE USER U1 IDENTIFIED BY XYZ ACCOUNT LOCK;
GRANT CREATE TABLE, CREATE VIEW, CREATE ROLE, CREATE SEQUENCE,
CREATE PROCEDURE, CREATE SESSION, UNLIMITED TABLESPACE TO U1;
GRANT INSERT, UPDATE ON EMP TO U1;
GRANT INSERT(EMPNO), UPDATE(SDATE, EDATE) ON VAC TO U1;
--------------------------------------------------------------------------------------------
CREATE PROCEDURE GET_CNT(MN NUMBER, CNT OUT NUMBER) IS
X NUMBER := 0;
CURSOR C1 IS SELECT E.ENAME FROM EMP E, EMP M WHERE M.EMPNO = E.MGR AND M.EMPNO = MN;
BEGIN
SELECT EMPNO INTO CNT FROM EMP WHERE EMPNO = MN;
X := X + 1;
SELECT M.EMPNO INTO CNT FROM EMP E, EMP M WHERE M.EMPNO = E.MGR AND E.EMPNO = MN;
FOR I IN C1 LOOP
DBMS_OUTPUT.PUT_LINE(I.ENAME);
CNT := C1%ROWCOUNT;
END LOOP;
...
CREATE ROLE R1;
GRANT CREATE TABLE, CREATE VIEW, CREATE ROLE, CREATE SEQUENCE,
CREATE PROCEDURE, CREATE SESSION, UNLIMITED TABLESPACE,
CREATE USER TO R1;
GRANT INSERT, UPDATE, DELETE, REFERENCES, INDEX
ON EMP TO R1;
SELECT * FROM ROLE_SYS_PRIVS;
SELECT * FROM ROLE_TAB_PRIVS;
REVOKE CREATE TABLE, CREATE VIEW FROM R1;
REVOKE INSERT, UPDATE ON EMP FROM R1;
--------------------------------------------------------------------------------------------
CREATE FUNCTION GET_CNT(MN NUMBER) RETURN NUMBER IS
CURSOR C1 IS SELECT E.ENAME FROM EMP E, EMP M WHERE M.EMPNO = E.MGR AND M.EMPNO = MN;
CNT NUMBER; X NUMBER := 0;
BEGIN
SELECT EMPNO INTO CNT FROM EMP WHERE EMPNO = MN;
X := X + 1;
SELECT COUNT(*) INTO CNT FROM EMP E, EMP M WHERE M.EMPNO = E.MGR AND
M.EMPNO = MN GROUP BY M.EMPNO;
FOR I IN C1 LOOP
DBMS_OUTPUT.PUT_LINE(I.ENAME);
END LOOP;
DBMS_OUTPUT.PUT_LINE(CNT);
...
--------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION FUN_CHECK(NUM1 NUMBER, NUM2 NUMBER) RETURN NUMBER IS
CNT NUMBER;
BEGIN
IF (NUM1 IS NOT NULL OR NUM1 != 0) AND (NUM2 IS NOT NULL OR NUM2 != 0) THEN
SELECT COUNT(*) INTO CNT FROM EMP WHERE TO_CHAR(HIRE_DATE, 'YYYY') = NUM1 AND
TO_CHAR(HIRE_DATE, 'MM') = NUM2;
ELSIF (NUM1 IS NOT NULL OR NUM1 != 0) AND (NUM2 IS NULL OR NUM2 = 0) THEN
SELECT COUNT(*) INTO CNT FROM EMP WHERE TO_CHAR(HIRE_DATE, 'YYYY') = NUM1;
ELSIF (NUM1 IS NULL OR NUM1 = 0) AND (NUM2 IS NOT NULL OR NUM2 != 0) THEN
SELECT COUNT(*) INTO CNT FROM EMP WHERE TO_CHAR(HIRE_DATE, 'YYYY') = TO_CHAR(SYSDATE, 'YYYY') AND
TO_CHAR(HIRE_DATE, 'MM') = NUM2;
END IF;
IF CNT = 0 THEN
RETURN NULL;
...
--------------------------------------------------------------------------------------------
CREATE PROCEDURE PRINT_VAC(EN NUMBER) IS
CURSOR C1 IS SELECT SDATE, EDATE FROM VAC WHERE EMPNO = EN;
X NUMBER := 0; ENO NUMBER;
BEGIN
SELECT EMPNO INTO ENO FROM EMP WHERE EMPNO = EN;
X := X + 1;
SELECT SUM(EDATE - SDATE) INTO ENO FROM VAC WHERE EMPNO = EN GROUP BY EMPNO;
FOR I IN LOOP
DBMS_OUTPUT.PUT_LINE(I.SDATE || ' ' || I.EDATE);
END LOOP;
EXCEPTION
WHEN NO_DATA_FOUND THEN 
IF X = 0 THEN DBMS_OUTPUT.PUT_LINE('NO EMPNO');
ELSE DBMS_OUTPUT.PUT_LINE('NO VAC');
END IF;
END;
SET SERVEROUTPUT ON
DECLARE
...
YEARS NUMBER := 2019;
BEGIN
D := TO_CHAR(SYSDATE, 'DD') || '/' || TO_CHAR(SYSDATE, 'MM') || '/' || YEARS;
INSERT INTO COPY_EMP(HIRE_DATE) VALUES(TO_DATE(D, 'DD/MM/YYYY'));
END;
SET SERVEROUTPUT ON
DECLARE
D VARCHAR2(100);
BEGIN
D := 1 || '/' || TO_CHAR(SYSDATE, 'MM/YYYY');
DBMS_OUTPUT.PUT_LINE(D);
END;
نفترض DE VARCHAER(100)
IF :NEW.HIRE_DATE IS NULL THEN
DE := 1 || '/' || TO_CHAR(SYSDATE, 'MM/YYYY');
:NEW.HIRE_DATE := TO_DATE(DE, 'DD/MM/YYYY');
END IF;
ALTER TABLE COPY_EMP ADD HIRE_DATE DATE
SELECT * FROM COPY_EMP
DROP TRIGGER TR1
PRAGMA
...
FIL UTL_FILE.FILE_TYPE;
BEGIN
FIL := UTL_FILE.FOPEN('DIR', 'MYFILE.TXT', 'W');
UTL_FILE.PUT_LINE(FIL, 'HELLO AHMED');
UTL_FILE.FCLOSE(FIL);
END;
SELECT COUNT(*) FROM EMP WHERE DEPTNO = 500 GROUP BY DEPTNO
DROP ROLE RE
CREATE ROLE RE
GRANT CREATE TABLE TO RE

CREATE PACKAGE PACK IS
FUNCTION CHK(ENO NUMBER, DT DATE, VCNT NUMBER) RETURN NUMBER;
PROCEDURE INST_VAC(RECVAC VAC%ROWTYPE);
END PACK;

CREATEOR REPLACE PACKAGE BODY PACK IS
FUNCTION CHK(ENO NUMBER, DT DATE, VCNT NUMBER) RETURN NUMBER IS
CURSOR C1 IS SELECT * FROM VAC WHERE EMPNO = ENO;
X NUMBER;
BEGIN
SELECT EMPNO INTO X FROM VAC WHERE EMPNO = ENO;
FOR I IN C1 LOOP
IF (DT < I.VDATE AND DT + VCNT > I.VDATE + T.CNT)
OR (DT < I.VDATE AND DT + VCNT > I.VDATE) 
OR (DT > I.VDATE AND DT + VCNT > I.VDATE + T.CNT) THEN
RETURN 1;
END LOOP;
RETURN 0;
EXCEPTION WHEN NO_DATA_FOUND THEN
RETURN -1;
END CHK;
PROCEDURE INS_VAC(RECVAC VAC%ROWTYPE) IS
EN NUMBER;
BEGIN
SELECT EMPNO INTO EN FROM EMP WHERE EMPNO = RECVAC.EMPNO;
IF CHK(RECVAC.EMPNO, RECVAC.VDATE, RECVAC.CNT) = 1 THEN
INSERT INTO VAC VALUES(RECVAC.EMPNO, RECVAC.VDATE, RECVAC.CNT);
END IF;
EXCEPTION WHEN NO_DATA_FOUND THEN
...
GRANT CREATE USER, CREATE TABLE, CREATE VIEW, CREATE ROLE, CREATE SESSION,
UNLIMITED TABLESPACE, CREATE PROCEDURE, CREATE SYNONYM, CREATE SEQUENCE TO SCOTT WITH ADMIN OPTION;
...
CREATE TRIGGER TR1 BEFORE DELETE OR INSERT ON EMP FOR EACH ROW
DECLARE
BEGIN
IF DELETING THEN
        IF :OLD.DEPTNO NOT NULL AND :OLD.SAL NOT NULL THEN
        RAISE_APPLICATION_ERROR(-20000, 'NO DELETING');
        END IF;
ELSIF INSERTING THEN
:NEW.ENAME := UPPER(:NEW.ENAME);
:NEW.HIRE_DATE := TO_DATE(SYSDATE, 'DD/MM/YYYY');
IF :NEW.DEPTNO IS NULL THEN
:NEW.DEPTNO := 40;
END IF;
...
(
  STUDNO NUMBER(3) CONSTRAINT PK_STUDENT PRIMARY KEY,
  SNAME VARCHAR2(100) NOT NULL,
  SCLASS NUMBER(2) NOT NULL,
  SPHONE NUMBER(10), DPHONE NUMBER(10), MPHONE NUMBER(10),
  STEACHER VARCHAR2(100) NOT NULL, NGROUP VARCHAR2(100) NOT NULL,
  CONSTRAINT PK_SNAME UNIQUE(SNAME)
);
INSERT INTO STUDENTS VALUES(1, 'أحمد دلوان', '12', '0936968795', '', '', 'وسام العمري', 'OmarBinKhattab');
DELETE FROM STUDENTS
CREATE TABLE SCHECK 
(
  STUDNO NUMBER(3) ,CONSTRAINT FK_STUDENT_CH FOREIGN KEY (STUDNO) REFERENCES STUDENTS(STUDNO),
  TODAY_IS_DATE DATE, PRESENCE NUMBER(1), PAGESNUM NUMBER(2,1), LVL VARCHAR2(50),
  HOMEWORK VARCHAR2(100), NOTES VARCHAR2(100)
);
ALTER TABLE SCHECK MODIFY PAGESNUM NUMBER(4,1)    
SELECT * FROM SCHECK WHERE STUDNO = 4
SELECT STUDNO, SNAME || ' (' || SCLASS || ') ' FULLNAME FROM STUDENTS;
SELECT * FROM STUDENTS
SELECT TO_CHAR(TODAY_IS_DATE, 'DAY') FROM SCHECK
DROP TABLE SCHECK
DROP TABLE STUDENTS
ALTER TABLE SCHECK DROP COLUMN PRESENCE
DESC STUDENTS
DESC SCHECK
CREATE TRIGGER STUDENTS_PK_ADD BEFORE INSERT ON STUDENTS FOR EACH ROW WHEN (NEW.STUDNO IS NULL)
DECLARE
CNT NUMBER;
CURSOR C1 IS SELECT STUDNO FROM STUDENTS;
BEGIN
FOR I IN 1..999 LOOP 
        CNT := I;
        FOR J IN C1 LOOP 
...
END LOOP;
END;
CREATE PACKAGE PACK_STUDENTS IS
PROCEDURE INS_STD()
END;
DELETE FROM SCHECK WHERE PAGESNUM = 1
SELECT * FROM SCHECK WHERE STUDNO = 4
UPDATE SCHECK SET PAGESNUM = 5 WHERE STUDNO = 4
SELECT * FROM STUDENTS ,SCHECK WHERE NGROUP = 'OmarBinKhattab' AND STUDENTS.STUDNO = SCHECK.STUDNO
DELETE FROM STUDENTS WHERE STUDNO = 1
UPDATE SCHECK SET NOTES = NULL WHERE STUDNO IN (4,11,7)
SELECT TO_CHAR(TODAY_IS_DATE, 'DAY') FROM SCHECK
SELECT SNAME "اسم الطالب" FROM STUDENTS
CREATE OR REPLACE VIEW VALL_ANAS_BIN_MALEK AS
SELECT STUDNO "الرقم", SNAME "الاسم", SCLASS "الصف", SPHONE "رقم هاتف", DPHONE "رقم الوالد", MPHONE "رقم الوالدة" FROM STUDENTS WHERE NGROUP = 'AnasBinMalek' ORDER BY SNAME
SELECT الصف FROM VALL_ANAS_BIN_MALEK
SELECT SNAME FROM STUDENTS ST, SCHECK SC WHERE ST.STUDNO = SC.STUDNO;
SELECT * FROM SCHECK
SELECT TO_CHAR(TODAY_IS_DATE, 'DAY') || TO_CHAR(TODAY_IS_DATE, 'DD/MM') DD, STUDNO, TO_CHAR(TODAY_IS_DATE, 'DD/MM/YY') FROM SCHECK
UNION
SELECT SNAME, STUDENTS.STUDNO,TO_CHAR(TODAY_IS_DATE, 'DD/MM/YY') FROM STUDENTS ,SCHECK WHERE STUDENTS.STUDNO = SCHECK.STUDNO
ORDER BY 1

SELECT TO_CHAR(TODAY_IS_DATE, 'DD/MM/YYYY'), TODAY_IS_DATE FROM SCHECK 
UNION
SELECT TO_CHAR(STUDNO) , TODAY_IS_DATE FROM SCHECK
ORDER BY TODAY_IS_DATE

SELECT TO_CHAR(DEPTNO), DEPTNO, 1 A FROM EMP
UNION
SELECT ENAME, DEPTNO, 2 FROM EMP
ORDER BY DEPTNO
SELECT * FROM STUDENTS
SELECT * FROM SCHECK
SELECT * FROM SCHECK WHERE STUDNO = 3 AND PAGESNUM = 2
UPDATE SCHECK SET NOTES = 'تقصير في الحفظ' WHERE STUDNO = 9 AND PAGESNUM = 2

SELECT * FROM STUDENTS WHERE NGROUP = 'AnasBinMalek';
SELECT TO_CHAR(TODAY_IS_DATE, 'DAY') || TO_CHAR(TODAY_IS_DATE, 'DD/MM') DA FROM SCHECK WHERE STUDNO IN (SELECT STUDNO FROM STUDENTS WHERE NGROUP = 'AnasBinMalek') GROUP BY TO_CHAR(TODAY_IS_DATE, 'DD/MM/YYYY')
SELECT TO_CHAR(TODAY_IS_DATE, 'DD/MM') || ' ' || TO_CHAR(TODAY_IS_DATE, 'DAY') DA , TO_CHAR(TODAY_IS_DATE, 'DD/MM/YYYY') FULDA FROM SCHECK WHERE STUDNO IN (SELECT STUDNO FROM STUDENTS WHERE NGROUP = 'AnasBinMalek') GROUP BY TO_CHAR(TODAY_IS_DATE, 'DD/MM'),TO_CHAR(TODAY_IS_DATE, 'DAY'),TO_CHAR(TODAY_IS_DATE, 'DD/MM/YYYY')
CREATE OR REPLACE VIEW VIEW_CHECK_STUDENT AS
SELECT SNAME "اسم_الطالب",STUDENTS.STUDNO, TO_CHAR(TODAY_IS_DATE, 'DD/MM/YYYY')  STUD_DATE,PAGESNUM "عدد_صفحات_مسمعة", LVL "تقدير", HOMEWORK "وظيفة" , NOTES "ملاحظات" FROM STUDENTS ,SCHECK WHERE STUDENTS.STUDNO = SCHECK.STUDNO
SELECT اسم الطالب,عدد_صفحات_مسمعة, تقدير,وظيفة, ملاحظات FROM VIEW_CHECK_STUDENT WHERE  STUD_DATE = '05/06/2022'
SELECT * FROM STUDENTS, SCHECK WHERE STUDENTS.STUDNO = SCHECK.STUDNO
SELECT STUD_DATE,عدد_صفحات_مسمعة, تقدير,وظيفة, ملاحظات FROM VIEW_CHECK_STUDENT WHERE STUDNO = 45
SELECT * FROM STUDENTS
CREATE VIEW VIEW_FULL_TASK AS
SELECT SNAME "اسم الطالب", SCLASS "الصف", SUM(PAGESNUM) "عدد_الصفحات_مسمعة"
, PREVIOUS_HISTORY "محفوظات_السابقة" FROM STUDENTS S, SCHECK C 
WHERE S.STUDNO = C.STUDNO AND NGROUP = 'AnasBinMalek' 
GROUP BY SNAME, SCLASS, PREVIOUS_HISTORY ORDER BY 3 DESC
SELECT DISTINCT STEACHER, NGROUP FROM STUDENTS
DESC STUDENTS
DESC SCHECK
SELECT TO_CHAR()
UPDATE SCHECK SET TODAY_IS_DATE = TO_DATE('06/06/2022', 'DD/MM/YYYY') WHERE STUDNO = 3 AND HOMEWORK = '20'
UPDATE SCHECK SET LVL = 'جيد جدا' WHERE LVL = 'جيدا جدا'
SELECT * FROM STUDENTS
SELECT SNAME FROM STUDENTS, SCHECK WHERE STUDENTS.STUDNO = SCHECK.STUDNO AND STUDENTS.STUDNO = 5 AND TO_CHAR(TODAY_IS_DATE, 'DD/MM/YYYY') = '12/06/2022'
SELECT * FROM STUDENTS WHERE STUDNO = 5
UPDATE STUDENTS SET
DELETE FROM SCHECK WHERE STUDNO = 14
SELECT * FROM SCHECK WHERE STUDNO = 14
INSERT INTO SCHECK VALUES(14, '12-JUN-22', 4, 'ممتاز', '4', '')
SELECT MAX(ENAME) FROM EMP
ALTER TABLE STUDENTS ADD Previous_History VARCHAR2(1000)
SELECT * FROM STUDENTS
SELECT EMPNO, ENAME, SAL, SUM(EDATE - SDATE), TO_CHAR(SDATE, 'DD/MM/YYYY') 
FROM EMP, VAC 
WHERE EMP.EMPNO = VAC.EMPNO 
SELECT EMPNO, TO_CHAR(SDATE, 'DD/MM/YYYY'), TO_CHAR(EDATE, 'DD/MM/YYYY') FROM VAC WHERE EMPNO = 190
SELECT TO_CHAR(SDATE, 'MM') FROM VAC
DROP VIEW V1

CREATE VIEW V1 AS
SELECT EMPNO, SDATE , SUM(EDATE - SDATE) SUMVAC FROM VAC GROUP BY EMPNO, SDATE
HAVING SUM(EDATE - SDATE) IN (SELECT MAX(SUM(EDATE - SDATE)) FROM VAC
WHERE EMPNO = 190 GROUP BY TO_CHAR(SDATE, 'DD/MM/YYYY'));
CREATE VIEW V2 AS
SELECT ENAME, EMP.EMPNO, SAL, SUMVAC, TO_CHAR(SDATE, 'YYYY') 
FROM EMP, V1 WHERE EMP.EMPNO = V1.EMPNO;

CREATE VIEW V1 AS
SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL IN (SELECT DISTINCT SAL FROM EMP E WHERE 3 > (SELECT COUNT(DISTINCT SAL) FROM EMP WHERE SAL > E.SAL));

SELECT * FROM COPY_EMP WHERE INSTR(ENAME,' ',1, 3) = 0
INSERT INTO COPY_EMP(ENAME) VALUES('A ME R')
...
CONNECT BY level <= 9;
DROP SEQUENCE SQU1
INSERT INTO COPY_EMP(EMPNO) VALUES(SQU1.NEXTVAL)
SELECT * FROM COPY_EMP

السؤال الأول
1- SELECT * FROM EMP WHERE (SUBSTR(ENAME, 1, 1) = SUBSTR(UPPER(ENAME), LENGTH(ENAME), 1) 
AND INSTR(ENAME,' ',1, 3) = 0 AND INSTR(ENAME,' ',1, 2) <> 0) OR (COMM IS NULL AND (SAL < 2000 OR SAL > 5000)) 
OR (JOB IN ('CLERK', 'DRIVER') AND DEPTNO IN (20, 10));

2- SELECT EMPNO FROM EMP WHERE EMPNO NOT IN (SELECT EMPNO FROM VAC WHERE TO_CHAR(SDATE, 'YYYY') = 2020 AND TO_CHAR(SDATE, 'MM') > 6);

3- SELECT ENAME, EMPNO FROM EMP WHERE EMPNO IN 
(SELECT EMPNO FROM VAC WHERE TO_CHAR(SDATE, 'YYYY') = TO_CHAR(SYSDATE, 'YYYY') 
GROUP BY EMPNO HAVING SUM(EDATE - SDATE) > 20);

4- SELECT E.ENAME, E.SAL, M.ENAME, M.JOB FROM EMP E, EMP M
WHERE M.EMPNO = E.MGR AND (E.SAL > 10000 OR E.JOB = 'ANALYST');

5- SELECT COUNT(*), SUM(SAL), GRADE FROM EMP, SALGRADE WHERE SAL(+) BETWEEN LOSAL AND HISAL
GROUP BY GRADE;

6- SELECT C.DNAME, S.DNAME FROM DEPT C, DEPT S WHERE C.DEPTNO IN (SELECT DEPTNO FROM EMP GROUP BY DEPTNO 
HAVING COUNT(*) = (SELECT MAX(COUNT(*)) FROM EMP GROUP BY DEPTNO)) AND
S.DEPTNO IN (SELECT DEPTNO FROM EMP GROUP BY DEPTNO 
HAVING SUM(SAL) = (SELECT MAX(SUM(SAL)) FROM EMP GROUP BY DEPTNO));

7- DELETE FROM VAC WHERE TO_DATE(SDATE, 'DD/MM/YYYY') BETWEEN TO_DATE('12/3/2020', 'DD/MM/YYYY') AND TO_DATE('15/5/2020', 'DD/MM/YYYY');

8- UPDATE EMP E SET SAL = SAL * 1.15 WHERE SAL < (SELECT AVG(SAL) FROM EMP WHERE DEPTNO = E.DEPTNO);

9- UPDATE EMP SET SAL = DECODE(DEPTNO, 10, SAL * 1.1, 20, SAL * 1.15, SAL * 1.05);

10- INSERT INTO BONUS SELECT ENAME, SAL, JOB, COMM FROM EMP WHERE JOB IS NULL OR DEPTNO IS NULL;

السؤال الثاني
1- CREATE TABLE EMP(
        EMPNO NUMBER(4) CONSTRAINT PK_EMP PRIMARY KEY,
        HIRE_DATE DATE DEFAULT(TO_DATE(SYSDATE, 'DD/MM/YYYY')),
        DEPTNO NUMBER(3) REFERENCES DEPT(DEPTNO) ON DELETE CASCADE
)

2- CREATE SEQUENCE SQU1 START WITH 40 MINVALUE -140 INCREMENT BY 18 MAXVALUE 45 CYCLE CACHE 10;
قيمة العداد بعد استخدامه لثلاث مرات هو
-122
قيمة ال
CACHE = (CEIL (MAXVALUE - MINVALUE)) / (INCREMENT) = (CEIL (45 - (-140))) / (18) = 10

3- ALTER TABLE DEPT DROP CONSTRAINT PK_DEPTNO;

السؤال الثالث
CREATE VIEW V1 AS
SELECT EMPNO, ENAME FROM EMP E WHERE 3 > (SELECT COUNT(DISTINCT SAL) FROM EMP WHERE SAL > E.SAL)

السؤال الرابع
CREATE VIEW V2 AS
SELECT EMPNO FROM VAC WHERE TO_CHAR(SDATE, 'MM') = 11 AND TO_CHAR(SDATE, 'YYYY') = 2019

SELECT DNAME FROM DEPT WHERE DEPTNO IN (SELECT COUNT(*) FROM EMP
GROUP BY DEPTNO, JOB HAVING COUNT(*) > 1 AND JOB = 'IT_PROG' 
OR COUNT(*) > 1 AND JOB = 'AD_VP')
SELECT * FROM EMP WHERE JOB IN ('AD_VP', 'IT_PROG')
DESC STUDENTS
SELECT * FROM STUDENTS WHERE STUDNO = 35
DELETE FROM STUDENTS WHERE STUDNO = 35

السؤال الأول
GRANT CREATE TABLE, CREATE VIEW, CREATE ROLE, CREATE PROCEDURE, CREATE SEQUENCE, UNLIMITED TABLESPACE,
CREATE SYNONYM, CREATE TRIGGER TO SCOTT WITH ADMIN OPTION;
CREATE PROFILE NEW_PROFILE LIMIT
SESSION_PER_USER 2 PASSWORD_LIFE_TIME 2 PASSWORD_LOCK_TIME 2 FAILED_LOGIN_ATTEMPTS 3;
REVOKE CREATE TABLE, CREATE VIEW FROM SCOTT;

السؤال الثاني
CREATE TRIGGER TR1 BEFORE DELETE OR INSERT ON EMP FOR EACH ROW
DECLARE MX NUMBER; EN NUMBER;
BEGIN
IF DELETING THEN
        IF :OLD.DEPTNO IS NOT NULL AND :OLD.SAL IS NOT NULL THEN
        RAISE_APPLICATION_ERROR(-20000, 'NO DELETE');
        END IF;
ELSIF INSERTING THEN
        :NEW.ENAME := UPPER(:NEW.ENAME);
        :NEW.HIRE_DATE := TO_CHAR(SYSDATE, 'DD/MM/YYYY');
        IF :NEW.DEPTNO IS NULL THEN
        :NEW.DEPTNO := 40;
        END IF;
        SELECT NVL(MAX(EMPNO), 1) INTO MX FROM EMP;
        FOR I IN 1..MX LOOP
        EN := I;
        SELECT EMPNO INTO EN FROM EMP WHERE EMPNO = I;
        END LOOP;
        :NEW.EMPNO := EN + 1;
        EXCEPTION
        WHEN NO_DATA_FOUND THEN
...
السؤال الثالث
CREATE FUNCTION GET_TAX(NUM NUMBER) RETURN NUMBER IS
SALARY NUMBER; TX NUMBER;
BEGIN
SELECT SAL INTO SALARY FROM EMP WHERE EMPNO = NUM;
IF SALARY <= 5000 THEN
        TX := SALARY * 0;
ELSIF SALARY <= 7000 THEN
        TX := (SALARY - 5000) * (2/100);
ELSIF SALARY <= 11000 THEN
        TX := (7000 - 5000) * (2/100) + (SALARY - 7000) * (5/100);
ELSIF SALARY <= 16000 THEN
        TX := (7000 - 5000) * (2/100) + (11000 - 7000) * (5/100) + (SALARY - 11000) * (7/100);
ELSIF SALARY > 16000 THEN
        TX := (7000 - 5000) * (2/100) + (11000 - 7000) * (5/100) + (16000 - 11000) * (7/100) + SALARY * (12/100);
END IF;
RETURN TX;
WHEN NO_DATA_FOUND THEN
RETURN NULL;
...
FOR I IN C1 LOOP
UTL_FILE.PUT_LINE(FIL, I.ENAME || ' ' || I.JOB);
END LOOP;
UTL_FILE.FCLOSE(FIL);
END;
...
السؤال الخامس
CREATE PACKAGE PAK1 IS
PROCEDURE PRINT_ENAME(DN NUMBER);
TYPE RES IS RECORD(EN EMP.ENAME%TYPE, JO EMP.JOB%TYPE);
TYPE T IS TABLE OF REC INDEX BY BINARY_INTEGER;
PROCEDURE GET_TAB(YA NUMBER, TAB OUT T);
END;

CREATE OR REPLACE PACKAGE BODY PAK1 IS
PROCEDURE PRINT_ENAME(DN NUMBER) IS
CURSOR C1 IS SELECT ENAME FROM EMP WHERE DEPTNO = DN;
X NUMBER := 0; Y NUMBER;
BEGIN
SELECT DEPTNO INTO Y FROM DEPT WHERE DEPTNO = DN;
X := X + 1;
SELECT COUNT(*) INTO Y FROM EMP WHERE DEPTNO = DN GROUP BY DEPTNO;
FOR I IN C1 LOOP
DBMS_OUTPUT.PUT_LINE(I.ENAME);
END LOOP;
SELECT SUM(SAL) INTO Y FROM EMP WHERE DEPTNO = DN;
DBMS_OUTPUT.PUT_LINE(Y);
EXCEPTION
WHEN NO_DATA_FOUND THEN
IF X = 0 THEN
DBMS_OUTPUT.PUT_LINE('NOT FOUND DEPT');
ELSE
DBMS_OUTPUT.PUT_LINE('NOT FOUND EMP');
END IF;
END;
PROCEDURE GET_TAB(YA NUMBER, TAB OUT T) IS
CURSOR C1 IS SELECT EMPNO, ENAME, JOB FROM EMP WHERE TO_CHAR(HIRE_DATE, 'YYYY') = YA;
BEGIN
FOR I IN C1 LOOP
TAB(I.EMPNO).EN := I.ENAME;
TAB(I.EMPNO).JO := I.JOB;
END LOOP;
END;
END;
الاستدعاء
...
X NUMBER;
BEGIN
PAK1.GET_TAB(2002, MYTAB);
X := MYTAB.FIRST;
FOR I IN 1..MYTAB.COUNT LOOP
DBMS_OUTPUT.PUT_LINE(MYTAB(X).EN || ' ' || MYTAB(X).JO);
X := MYTAB.NEXT(X);
END LOOP;
END;

السؤال الأول
CREATE USER SCOTT IDENTIFIED BY XYZ QUOTA 5M ON USERS PASSWORD EXPIRED;
GRANT CREATE USER, ALTER USER, DROP USER, CREATE SESSION, CREATE SEQUENCE, UNLIMITED TABLESPACE,
CREATE SYNONYM, CREATE TRIGGER TO SCOTT WITH ADMIN OPTION;
GRANT CREATE PROCEDURE, CREATE TABLE, CREATE VIEW, CREATE ROLE TO Finance;
GRANT SELECT, INSERT, UPDATE, DELETE, ALTER ON EMP TO Finance;
GRANT INSERT(DNAME, DEPTNO), UPDATE(DNAME) ON DEPT TO Finance;
USER_SOURCE عرض رماز المصدري للكائنات المخزنة التي يمتلكها المستخدم الحالي
USER_TAB_PRIVS_RECD عرض سماحيات للكائن الممنوحة للمستخدم الحالي
CREATE PROFILE NEW_PROFILE LIMIT
SESSION_PER_USER 2 PASSWORD_LIFE_TIME 2 PASSWORD_LOCK_TIME 2 FAILED_LOGIN_ATTEMPTS 3;

السؤال الثاني
CREATE TRIGGER TR1 BEFORE INSERT OR UPDATE OR DELETE ON EMP FOR EACH ROW
DECLARE MX NUMBER; EN NUMBER;
BEGIN
IF DELETING THEN
IF :OLD.DEPTNO IS NOT NULL OR :OLD.JOB IS NOT NULL THEN
RAISE_APPLICATION_ERROR(-20000, 'NO DELETING');
END IF;
ELSIF INSERTING THEN
:NEW.ENAME := UPPER(:NEW.ENAME);
...
END LOOP;
:NEW.EMPNO := EN + 1;
ELSIF UPDATING('EMPNO') OR UPDATING('DEPTNO') OR UPDATING('JOB') OR UPDATING('HIRE_DATE') THEN
RAISE_APPLICATION_ERROR(-20001, 'NO UPDATING');
END IF;
EXCEPTION
WHEN NO_DATA_FOUND THEN
:NEW.EMPNO := EN;
...
السؤال الثالث
CREATE FUNCTION GET_TAX(ENO NUMBER) RETURN NUMBER IS
EMP_SAL NUMBER; X NUMBER := 0; NUMBER Y;
CURSOR C1 IS SELECT * FROM TAXES WHERE EMP_SAL >= LOLEVEL;
BEGIN
SELECT EMPNO INTO Y FROM EMP WHERE EMPNO = ENO;
X := X + 1;
SELECT SAL INTO EMP_SAL FROM EMP WHERE EMPNO = ENO;
X := X + 1;
SELECT SAL INTO Y FROM EMP, TAXES WHERE EMPNO = ENO AND SAL BETWEEN LOLEVEL AND HILEVEL;
Y := NULL;
FOR I IN C1 LOOP
IF EMP_SAL > I.HILEVEL THEN
Y := (I.HILEVEL - I.LOLEVEL + 1) * (I.TAX / 100) + Y;
ELSE
Y := (EMP_SAL - I.LOLEVEL + 1) * (I.TAX / 100) + Y;
END LOOP;
RETURN Y;
EXCEPTION WHEN NO_DATA_FOUND THEN
IF X = 0 THEN
...
TYPE REC1 IS RECORD(DN DEPT.DNAME%TYPE, CNT NUMBER, SUMSAL NUMBER);
FUNCTION INTO_DEPT(DNO NUMBER) RETURN REC1;
TYPE REC2 IS RECORD(SD VAC.SDATE%TYPE, ED VAC.EDATE%TYPE);
TYPE TAB IS TABLE OF REC2 INDEX BY BINARY_INTEGER;
MYTAB TAB;
PROCEDURE SAVR_VAC(ENO NUMBER);
PROCEDURE PRINT;
END PACK1;

CREATE OR REPLACE PACKAGE BODY PACK1 IS
FUNCTION INFO_DEPT(DNO NUMBER) RETURN REC1 IS
MYREC REC1;
BEGIN
SELECT DNAME, COUNT(*), SUM(SAL) INTO MYREC.DN, MYREC.CNT, MYREC.SUMSAL FROM EMP, DEPT
WHERE EMP.DEPTNO = DEPT.DEPTNO AND DEPT.DEPTNO = DNO
GROUP BY DNAME;
RETURN MYREC;
END INFO_DEPT;
PROCEDURE SAVR_VAC(ENO NUMBER) IS
CURSOR C1 IS SELECT SDATE, EDATE FROM VAC WHERE EMPNO = ENO;
X NUMBER := 1;
BEGIN
FOR I IN C1 LOOP
MYTAB(X).SD := I.SDATE;
...
X NUMBER := MYTAB.FIRST;
BEGIN
FOR I IN MYTAB.COUNT LOOP
DBMS_OUTPUT.PUT_LINE(MYTAB(X).SD || ' ' || MYTAB(X).ED);
X := MYTAB.NEXT(X);
END LOOP;
END PRINT;
END PACK1;
...
MYREC PACK1.REC1;
BEGIN
MYREC := PACK1.INFO_DEPT(10);
DBMS_OUTPUT.PUT_LINE(MYREC.DN || ' ' || MYREC.CNT || ' ' || MYREC.SUMSAL);
END;

CREATE TRIGGER TR1 INSTEAD OF INSERT ON V1 FOR EACH ROW
DECLARE MX NUMBER; DNO NUMBER;
BEGIN
SELECT NVL(MAX(EMPNO), 0) INTO MX FROM EMP; MX := MX + 1;
IF :NEW.DNAME IS NULL AND :NEW.ENAME IS NOT NULL THEN
INSERT INTO EMP(EMPNO, ENAME, DEPTNO, SAL, COMM) VALUES(MX, :NEW.ENAME, 10, :NEW.TOTAL * 0.7, :NEW.TOTAL * 0.3);
ELSE IF :NEW.DNAME IS NOT NULL AND :NEW.ENAME IS NOT NULL THEN
SELECT DEPTNO INTO DNO FROM DEPT WHERE DNAME = :NEW.DNAME;
INSERT INTO EMP(EMPNO, ENAME, DEPTNO, SAL, COMM) VALUES(MX, :NEW.ENAME, DNO, :NEW.TOTAL * 0.7, :NEW.TOTAL * 0.3);
ELSIF :NEW.ENAME IS NULL THEN
RAISE_APPLICATION_ERROR(-20000, 'ERROR');
END IF;
EXCEPTION WHEN NO_DATA_FOUND THEN
INSERT INTO EMP(EMPNO, ENAME, DEPTNO, SAL, COMM) VALUES(MX, :NEW.ENAME, NULL, :NEW.TOTAL * 0.7, :NEW.TOTAL * 0.3);
END;

السؤال الأول
GRANT CREATE SESSION, CREATE TABLE, UNLIMITED TABLESPACE TO SCOTT;
ALTER USER SCOTT PASSWORD EXPIRED;
GRANT CREATE PROCEDURE, CREATE VIEW, CREATE ROLE, CREATE USER, ALTER USER, 
DROP USER, CREATE SEQUENCE, CREATE SYNONYM, CREATE TRIGGER TO Finance WITH ADMIN OPTION;
CREATE DIRECTORY DIR AS 'C:\temp';
GRANT READ, WRITE ON DIRECTORY DIR TO SCOTT;

السؤال الثاني
CREATE TRIGGER TR1 BEFORE INSERT OR UPDATE ON EMP FOR EACH ROW
DECLARE X NUMBER := 0; MX NUMBER; EN NUMBER;
CURSOR C1 IS SELECT EMPNO FROM EMP;
BEGIN
IF INSERTING THEN
IF :NEW.DEPTNO IS NULL THEN
...
السؤال الثالث
CREATE TRIGGER TR2 BEFORE DROP OR ALTER OR GRANT ON DATABASE
DECLARE P ORA_NAME_LIST_T; PN NUMBER;
BEGIN
IF ORA_SYSEVENT IN ('DROP', 'ALTER') AND ORA_DICT_OBJ_TYPE = 'TABLE'
AND ORA_DICT_OBJ_NAME IN ('EMP', 'DEPT') THEN
RAISE_APPLICATION_ERROR(-20000, 'ERROR IN DROP OR ALTER IN TABLES (EMP, DEPT)');
ELSIF ORA_SYSEVENT = 'GRANT' THEN
PX := ORA_PRIVILEGE_LIST(P);
FOR I IN 1..PN LOOP
IF P(I) IN ('INSERT', 'UPDATE', 'DELETE') AND ORA_DICT_OBJ_TYPE = 'TABLE'
AND ORA_DICT_OBJ_NAME = 'SALGRADE' AND USER IN ('SCOTT', 'TIGER') THEN
RAISE_APPLICATION_ERROR(-20001, 'ERROR IN GRANT');
END IF;
END LOOP;
END IF;
END;
...
السؤال الرابع
CREATE PACKAGE PACK1 IS
FUNCTION GET_CNT_DEPT RETURN NUMBER;
FUNCTION GET_CNT_VAC_2021(EN NUMBER) RETURN NUMBER;
PROCEDURE SAL_OVER(JO VARCHAR2, OVER NUMBER, CNT OUT NUMBER);
TYPE REC IS RECORD(RECEMP EMP%ROWTYPE, GD SALGRADE.GRADE%TYPE);
PROCEDURE GET_REC_EMP(EN NUMBER, MYREC OUT REC);
END;

السؤال الخامس
CREATE OR REPLACE PACKAGE BODY PACK1 IS
BEGIN
FUNCTION GET_CNT_DEPT RETURN NUMBER IS
CNT NUMBER;
BEGIN
SELECT COUNT(COUNT(EMPNO)) INTO CNT FROM EMP, DEPT WHERE EMP.DEPTNO(+) = DEPT.DEPTNO 
GROUP BY DEPT.DEPTNO HAVING COUNT(EMPNO) < 2;
RETURN CNT;
END GET_CNT_DEPT;
FUNCTION GET_CNT_VAC_2021(EN NUMBER) RETURN NUMBER IS
SUMVAC NUMBER; X NUMBER := 0;
BEGIN
SELECT EMPNO INTO SUMVAC FROM EMP WHERE EMPNO = 190;
X := X + 1;
SELECT SUM(EDATE - SDATE) INTO SUMVAC FROM VAC WHERE EMPNO = 190 AND TO_CHAR(SDATE, 'YYYY') = 2021;
RETURN SUMVAC;
EXCEPTION WHEN NO_DATA_FOUND THEN
IF X = 0 THEN RETURN NULL; ELSE RETURN 0; END IF;
END GET_CNT_VAC_2021;
PROCEDURE SAL_OVER(JO VARCHAR2, OVER NUMBER, CNT OUT NUMBER) IS
BEGIN
IF OVER IS NULL OR OVER < 1 THEN
DBMS_OUTPUT.PUT_LINE('ERROR IN UPDATE SALARY');
CNT := 0;
ELSE
UPDATE EMP SET SAL = SAL + OVER WHERE JOB = JO;
CNT := SQL%ROWCOUNT;
END IF;
END SAL_OVER;
PROCEDURE GET_REC_EMP(EN NUMBER, MYREC OUT REC) IS
BEGIN
SELECT EMP.*, GRADE INTO MYREC.RECEMP, MYREC.GD FROM EMP, SALGRADE 
WHERE EMPNO = EN AND SAL BETWEEN LOSAL AND HISAL;
END GET_REC_EMP;
END PACK1;
...
MYREC PACK1.REC;
BEGIN
PACK1.GET_REC_EMP(7200, MYREC);
DBMS_OUTPUT.PUT_LINE(MYREC.RECEMP.ENAME || ' ' || MYREC.GD);
END;

SELECT * FROM EMP WHERE HIRE_DATE < (SELECT MIN(HIRE_DATE) FROM EMP WHERE ENAME = 'Steven')
SELECT * FROM EMP WHERE SAL < (SELECT SAL FROM EMP WHERE ENAME = 'Steven' AND DEPTNO IN (90, 20))
SELECT * FROM EMP WHERE EMPNO IN (SELECT MGR FROM EMP) AND MGR IS NOT NULL
SELECT * FROM EMP
SELECT * FROM VAC
SELECT DISTINCT JOB, ENAME, EMPNO FROM EMP ORDER BY 2
SELECT * FROM EMP WHERE ENAME = '%%'
SELECT ENAME FROM EMP, VAC WHERE EMP.EMPNO = VAC.EMPNO GROUP BY TO_CHAR(SDATE, 'YYYY')
SELECT * FROM EMP WHERE 1
SELECT E.* FROM EMP E, EMP M WHERE M.EMPNO = E.MGR AND (SUBSTR(UPPER(E.ENAME), 1, 1) = SUBSTR(UPPER(E.ENAME), -1, 1) OR SUBSTR(UPPER(E.ENAME), 1, 1) = SUBSTR(UPPER(M.ENAME), -1, 1))
CREATE SEQUENCE SQU START WITH 300 MINVALUE 10 INCREMENT BY -15 MAXVALUE 350 CYCLE;
UPDATE EMP SET DEPTNO = 20 WHERE EMPNO NOT IN (SELECT EMPNO FROM VAC)
UPDATE EMP E SET SAL = SAL * 1.2 WHERE SAL < (SELECT AVG(SAL) FROM EMP WHERE DEPTNO = E.DEPTNO)
SELECT ENAME, EMPNO FROM EMP WHERE EMPNO NOT IN (SELECT EMPNO FROM VAC )
CREATE VIEW V1 AS
SELECT DEPTNO FROM EMP WHERE JOB IS NULL GROUP BY DEPTNO HAVING COUNT(*) > 15;
CREATE VIEW V2 AS
SELECT DNAME, DEPT.DEPTNO FROM DEPT, V1 WHERE DEPT.DEPTNO = V1.DEPTNO;
SELECT * FROM EMP WHERE SAL IN (SELECT MAX(SAL) FROM EMP WHERE SAL NOT IN (SELECT MAX(SAL) FROM EMP))
CREATE VIEW V1 AS
SELECT GRADE, MAX(COUNT(*)) FROM EMP, SALGRADE WHERE SAL BETWEEN LOSAL AND HISAL GROUP BY GRADE;
CREATE VIEW V2 AS
SELECT EMP.* FROM EMP, SALGRADE, V1 WHERE SAL BETWEEN LOSAL AND HISAL AND SALGRADE.GRADE = V1.GRADE