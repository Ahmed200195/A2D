...
WHERE M.ID_MINISTRY = E.ID_M;
CREATE OR REPLACE PACKAGE PAK_MINI_UNIV IS
FUNCTION FUNCHECK(CH VARCHAR2, NCH NUMBER, NA VARCHAR2) RETURN NUMBER;
END;

SET SERVEROUTPUT ON
CREATE OR REPLACE PACKAGE BODY PAK_MINI_UNIV IS
    FUNCTION FUNCHECK(CH VARCHAR2, NCH NUMBER, NA VARCHAR2) RETURN NUMBER IS
    CURSOR CURMINI IS SELECT MINISTRY_NAME,  EMAIL, PHONE FROM MINISTRY , EMAILPHONE
    WHERE ID_MINISTRY = ID_M;
    CURSOR CURUNIV IS SELECT UNIVERSITY_NAME || ' (' || CNAME || ')' AS FLUNIVCNTY,  EMAIL, PHONE FROM UNIVERSITY, EMAILPHONE, COUNTRY
    WHERE ID_UNIVERSITY = ID_M AND UNIVERSITY_COUNTRY = CVALUE;
    BEGIN
    IF NCH = 0 THEN
        IF NA = 'NAME' THEN
...
END;
SELECT * FROM EMAILPHONE
CREATE PROCEDURE INS_MINI_UNIV(MNAME VARCHAR2, CNTRY VARCHAR2,
G_EMAIL VARCHAR2, M_PHONE VARCHAR2, PASS VARCHAR2) IS 
ID NUMBER := SEQMINI.NEXTVAL;
...
FOR FM IN CM
    ...
    END LOOP;
ELSE
SELECT MIN(DNUM_U) INTO DEL_ID FROM DELETENUM;
FOR FU IN CU
    LOOP
        IF FU.ID_UNVERSITY = ID THEN
        ID := DEL_ID;
        END IF;
    END LOOP;
END IF;
