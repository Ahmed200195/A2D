...
CREATE OR REPLACE PACKAGE BODY PAK_MINI_UNVI IS
    ...
    ID NUMBER;
    E_ID NUMBER := SEQEML.NEXTVAL;
    CURSOR CM IS SELECT ID_MINISTRY FROM MINISTRY;
    CURSOR CU IS SELECT ID_UNIVERSITY FROM UNIVERSITY;
    CURSOR CEM IS SELECT EMPHNO FROM EMAILPHONE;
    BEGIN
    FOR EG IN CEM
    LOOP
        IF EG.EMPHNO = E_ID THEN
        SELECT MIN(DNUM_E) INTO E_ID FROM DELETENUM;
        DELETE FROM DELETENUM WHERE DNUM_E = E_ID;
        EXIT;
        END IF;
    END LOOP;
    ...
    FOR FM IN CM
        LOOP
            ...
            SELECT MIN(DNUM_M) INTO ID FROM DELETENUM;
            DELETE FROM DELETENUM WHERE DNUM_M = ID;
            EXIT;
            END IF;
        END LOOP;
        INSERT INTO MINISTRY VALUES(ID, 1, MUNAME);
        INSERT INTO EMAILPHONE(EMPHNO, EMAIL, ID_M, PHONE, PASSWORD)
        VALUES(E_ID, G_EMAIL, ID, MU_PHONE, PASS);
    ELSE
    ID := SEQUNVI.NEXTVAL;
    FOR FU IN CU
        LOOP
            IF FU.ID_UNIVERSITY = ID THEN
            SELECT MIN(DNUM_U) INTO ID FROM DELETENUM;
            DELETE FROM DELETENUM WHERE DNUM_U = ID;
            EXIT;
            END IF;
        END LOOP;
        INSERT INTO UNIVERSITY VALUES(ID, 1, MUNAME, CNTRY);
    ...
    CURSOR CURUNIV IS SELECT UNIVERSITY_NAME || ' (' || CNAME || ')' AS FLUNIVCNTY,  EMAIL, PHONE FROM UNIVERSITY, EMAILPHONE, COUNTRY WHERE ID_UNIVERSITY = ID_M AND UNIVERSITY_COUNTRY = CVALUE;
    CURSOR CURMINI IS SELECT MINISTRY_NAME,  EMAIL, PHONE FROM MINISTRY, EMAILPHONE WHERE ID_MINISTRY = ID_M;
    BEGIN
    IF NCH = 0 THEN
        IF NA = 'NAME' THEN
            FOR I IN CURMINI LOOP IF CH = I.MINISTRY_NAME THEN RETURN 1; END IF; END LOOP;
        ELSIF NA = 'EMAIL' THEN
            FOR I IN CURMINI LOOP IF CH = I.EMAIL THEN RETURN 1; END IF; END LOOP;
        ELSIF NA = 'PHONE' THEN
            FOR I IN CURMINI LOOP IF CH = I.PHONE THEN RETURN 1; END IF; END LOOP;
        END IF;
    ELSIF NCH = 1 THEN
        IF NA = 'NAME' THEN
            FOR I IN CURUNIV LOOP IF CH = I.FLUNIVCNTY THEN RETURN 1; END IF; END LOOP;
        ELSIF NA = 'EMAIL' THEN
            FOR I IN CURUNIV LOOP IF CH = I.EMAIL THEN RETURN 1; END IF; END LOOP;
        ELSIF NA = 'PHONE' THEN
            FOR I IN CURUNIV LOOP IF CH = I.PHONE THEN RETURN 1; END IF; END LOOP;
        END IF;
    RETURN 0;
    END IF;
    RETURN 0;
