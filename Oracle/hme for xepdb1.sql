...
END LOOP;
END;
CREATE TRIGGER GRD_ADD_PK BEFORE INSERT ON GRADUATE FOR EACH ROW WHEN (NEW.ID_GRADUATE IS NULL)
DECLARE
CNT NUMBER;
CURSOR C1 IS SELECT ID_GRADUATE FROM GRADUATE;
BEGIN
/*SAIZ DATA TYPE NUMBER(4) => 1..9999 */
FOR I IN 1..9999 LOOP CNT := I;
        FOR J IN C1 LOOP IF I = J.ID_GRADUATE THEN CNT := NULL; EXIT; END IF; END LOOP;
        IF CNT IS NOT NULL THEN :NEW.ID_GRADUATE := CNT; EXIT; END IF;
END LOOP;
END;
